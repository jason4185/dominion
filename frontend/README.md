...
This frontend reads protocol state from the deployed Dominion contract and uses the injected browser wallet for writes. The canonical contract address is exported by `src/lib/dominion/contractAdapter.ts`; it is deployed on GenLayer Bradbury Testnet (chain `4221`, RPC `https://rpc-bradbury.genlayer.com`). GEN amounts are converted with `1 GEN = 10^18` base units, and `market_start` is sent as Unix seconds.
...
- Use the real contract adapter for protocol data; keep presentation metadata local and static.
...
- injected-wallet connection with truncated address and GEN balance
...
CONTRACT DATA LAYER
Use the typed contract adapter and deployed contract reads for:

- markets, positions, pools, settlement evidence, claims, refunds, and activity
- no app-owned protocol persistence
...
- claim
- claimRefund
  These map to the corresponding Dominion contract views and writes.
...
- amount input in GEN
- quick buttons +1, +5, +10, MAX connected-wallet balance
- minimum 1 GEN copy
...
- confirmed writes refetch contract state and update UI/toasts
...
The notification bell reads compact on-chain wallet activity records for:

- BET_PLACED
- BET_TOPPED_UP
- PAYOUT_CLAIMED
- REFUND_CLAIMED
  Derived position states remain available through the portfolio and market reads. No standalone activity route or on-chain read-state write is used.
...
- confirmed creation success toast and route to the contract-created market
...
FRONTEND READ UX
Expose frontend-ready fields from contract reads for:
...
NOTIFICATION MODEL
Use on-chain activity records only. The bell displays recent activity and a count; no app-owned notification persistence or backend is used.

INTERACTION QUALITY

- All buttons and filters should work against contract-backed state.
- Search should work.
- category/status filters should work.
- market cards navigate correctly.
- wallet position top-up restrictions should be validated by the contract and reflected after refetch.
- claim/refund buttons refetch confirmed contract state and toast success.
- create-market flow should use the contract's numeric Unix timestamp ABI.
- no dead nav links.
- no blank screens.
