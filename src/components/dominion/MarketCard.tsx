...
export function MarketCard({ market, now }: { market: MarketView; now: number }) {
    ...
    >
      ...
      <div className="mt-3 flex items-end justify-between gap-3">
        <div className="min-w-0">
          <p className="num text-sm font-semibold text-foreground">{utcWindow(market.startMs, market.endMs)}</p>
      ...
      <div className="mt-3 space-y-1.5">
        {market.assets.map((a, i) => {
          const selected = market.userSelectedAsset === a.symbol;
          ...
          return (
            ...
            >
              ...
              <span className="num w-14 shrink-0 text-xs font-semibold text-foreground">{a.symbol}</span>
              <span className="min-w-0 flex-1 truncate text-[11px] text-muted-foreground">{a.company}</span>
              {selected && <Check className="size-3 shrink-0 text-primary-glow" />}
              <span className="num shrink-0 text-[11px] text-muted-foreground">{gen(market.pools[a.symbol], 0)}</span>
              <span className="num w-11 shrink-0 text-right text-[11px] font-semibold text-foreground">
                {pct(market.poolShares[a.symbol], 0)}
              </span>
            </div>
          );
        })}
      ...
      <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
        <span className="text-[11px] text-muted-foreground">
          {market.userStake > 0 ? `Your stake ${gen(market.userStake)} GEN` : "No position"}
        </span>
